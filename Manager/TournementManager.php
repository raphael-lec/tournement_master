...
class TournamentManager 
{
    ...
    {
        $query = $this->db->prepare("DELETE FROM tournement WHERE id = :id");
    ...
    {
        $query = $this->db->query("
            SELECT tor.*, g.game_name 
            FROM tournement tor
            LEFT JOIN game g ON g.id = tor.game_id
        ");
    ...
    {
        $query = $this->db->prepare("
            INSERT INTO tournement (name, format, date, owner, max_participant, status, discord_link, game_id) 
            VALUES (:name, :format, :date, :owner, :max_participant, :status, :discord_link, :game_id)
        ");
    ...
    public function setTournamentResult(int $tournamentId, int $winnerTeamId, int $runnerUpTeamId) : void
    {
        $query = $this->db->prepare("
            UPDATE tournement 
            SET winner_team_id = :winner, runner_up_team_id = :runner_up, status = 'finish'
            WHERE id = :id
        ");
        $query->execute([
            'winner'    => $winnerTeamId,
            'runner_up' => $runnerUpTeamId,
            'id'        => $tournamentId
        ]);
    }

    public function getTournamentsByUser(int $userId) : array
    {
        $query = $this->db->prepare("
            SELECT DISTINCT tor.*, g.game_name
            FROM tournement tor
            LEFT JOIN game g ON g.id = tor.game_id
            JOIN team t ON t.tournement_id = tor.id
            JOIN team_member tm ON tm.team_id = t.id
            WHERE tm.user_id = :uid
            ORDER BY tor.date DESC
        ");
        $query->execute(['uid' => $userId]);
        return $query->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    public function getMatchHistoryByUser(int $userId) : array
    {
        $query = $this->db->prepare("
            SELECT p.*, 
                   t1.name as team1_name, t1.id as team1_id,
                   t2.name as team2_name, t2.id as team2_id,
                   tm_user.team_id as user_team_id,
                   tor.name as tournament_name,
                   p.phase
            FROM party p
            JOIN party_participant pp1 ON pp1.party_id = p.id
            JOIN team t1 ON t1.id = pp1.team_id
            LEFT JOIN party_participant pp2 ON pp2.party_id = p.id AND pp2.team_id != pp1.team_id
            LEFT JOIN team t2 ON t2.id = pp2.team_id
            JOIN party_participant pp_user ON pp_user.party_id = p.id
            JOIN team_member tm_user ON tm_user.team_id = pp_user.team_id AND tm_user.user_id = :uid
            JOIN tournement tor ON tor.id = p.tournement_id
            WHERE p.score_team_1 IS NOT NULL
            GROUP BY p.id
            ORDER BY p.id DESC
        ");
        $query->execute(['uid' => $userId]);
    ...
    {
        $matches = $this->getMatchHistoryByUser($userId);
        $wins = 0;
        $losses = 0;
        $total = 0;

        foreach ($matches as $match) {
            if ($match['winner_team_id'] == 0) continue;
            $total++;
            if ($match['winner_team_id'] == $match['user_team_id']) {
                $wins++;
            } else {
                $losses++;
            }
